...
namespace
{
	bool BuildAndVerifyGeneratedShaderSource(const char* const i_path_source, const char* const i_path_target,
		const Tools::ShaderBuilder::eShaderType i_shaderType, const std::string& i_source);
...
bool Tools::ShaderBuilder::cShaderBuilder::Build(const ShaderBuilder::eShaderType i_shaderType, const std::vector<std::string>& i_arguments)
{
...
namespace
{
		...
		const Tools::ShaderBuilder::eShaderType i_shaderType, const std::string& i_source)
	{
		// Load any required OpenGL extensions
		{
			...
			{
				Tools::AssetBuildLibrary::UtilityFunctions::OutputErrorMessage(errorMessage.c_str(), i_path_source);
				return false;
			}
		}
		...
		{
			std::string errorMessage;
			if (!Engine::Windows::OGL::CreateHiddenContextWindow(hInstance, hiddenWindowInfo, &errorMessage))
			{
				wereThereErrors = true;
				Tools::AssetBuildLibrary::UtilityFunctions::OutputErrorMessage(errorMessage.c_str(), i_path_source);
				goto OnExit;
		...
		// Determine which vendor makes the GPU
		{
			...
			else
			{
				...
				Tools::AssetBuildLibrary::UtilityFunctions::OutputErrorMessage(errorMessage.str().c_str(), i_path_source);
				goto OnExit;
			}
		}
		// Verify that compiling shaders at run-time is supported
		{
			...
			{
				Tools::AssetBuildLibrary::UtilityFunctions::OutputErrorMessage("Compiling shaders at run-time isn't supported on this implementation"
		...
		GLuint shaderId = 0;
		{
			...
			shaderId = glCreateShader(shaderType);
			{
				...
				{
					wereThereErrors = true;
					Tools::AssetBuildLibrary::UtilityFunctions::OutputErrorMessage("OpenGL failed to get an unused shader ID", i_path_source);
					goto OnExit;
				}
			}
					...
					if (errorCode == GL_NO_ERROR)
					{
						...
						else
						{
							...
							errorMessage << "OpenGL failed to get compilation info of the shader source code: " <<
								reinterpret_cast<const char*>(gluErrorString(errorCode));
							Tools::AssetBuildLibrary::UtilityFunctions::OutputErrorMessage(errorMessage.str().c_str(), i_path_target);
							goto OnExit;
						}
					}
					else
	...
	OnExit:
		...
		{
			std::string errorMessage;
			if (!Engine::Windows::OGL::FreeHiddenContextWindow(hInstance, hiddenWindowInfo, &errorMessage))
			{
				wereThereErrors = true;
				Tools::AssetBuildLibrary::UtilityFunctions::OutputErrorMessage(errorMessage.c_str(), i_path_source);
			}
	...
	bool PreProcessShaderSource(const char* const i_path_source, std::string& o_shaderSource_preProcessed)
	{
		...
		if (result == 0)
		{
			o_shaderSource_preProcessed = mcpp_get_mem_buffer(static_cast<OUTDEST>(External::MCPP::OUTDEST::Out));
		}
		else
		{
			wereThereErrors = true;
			std::cerr << mcpp_get_mem_buffer(static_cast<OUTDEST>(External::MCPP::OUTDEST::Err));
			goto OnExit;
		}
	...
	bool SaveGeneratedShaderSource(const char* const i_path, const std::string& i_shader)
	{
		...
		{
			Tools::AssetBuildLibrary::UtilityFunctions::OutputErrorMessage(errorMessage.c_str(), i_path);
			return false;
		}
	}
}