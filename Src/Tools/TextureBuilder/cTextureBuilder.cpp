...
bool Tools::TextureBuilder::cTextureBuilder::Build(const std::vector<std::string>&)
{
	...
	// Initialize COM
	{
		...
		else
		{
			...
			AssetBuildLibrary::UtilityFunctions::OutputErrorMessage("DirextXTex couldn't be used because COM couldn't be initialized", m_path_source);
			goto OnExit;
		}
	}
...
namespace
{
		...
		DirectX::ScratchImage &o_texture)
	{
		...
		if (DirectX::IsCompressed(io_sourceImageThatMayNotBeValidAfterThisCall.GetMetadata().format))
		{
			...
			if (FAILED(result))
			{
				Tools::AssetBuildLibrary::UtilityFunctions::OutputErrorMessage("DirectXTex failed to uncompress source image", i_path);
				return false;
			}
		}
		else
...
#elif defined (OGL_API)
		{
			...
			if (FAILED(result))
			{
				Tools::AssetBuildLibrary::UtilityFunctions::OutputErrorMessage("DirectXTex failed to flip the source image vertically", i_path);
				return false;
			}
		}
#endif
		...
		DirectX::ScratchImage resizedImage;
		{
			...
			size_t targetHeight = flippedMetadata.height;
			{
				...
				// ("BC" stands for "block compression", and each block is 4x4)
				{
					...
					targetWidth = Engine::Math::RoundUpToMultiple_PowerOf2(targetWidth, blockSize);
					targetHeight = Engine::Math::RoundUpToMultiple_PowerOf2(targetHeight, blockSize);
				}
		...
		DirectX::ScratchImage imageWithMipMaps;
		{
			...
			if (FAILED(result))
			{
				Tools::AssetBuildLibrary::UtilityFunctions::OutputErrorMessage("DirectXTex failed to generate MIP maps", i_path);
				return false;
			}
		}
		// Compress the texture
		{
			...
			if (FAILED(result))
			{
				Tools::AssetBuildLibrary::UtilityFunctions::OutputErrorMessage("DirectXTex failed to compress the texture", i_path);
				return false;
			}
		}
	...
	bool LoadSourceImage(const char *const i_path, DirectX::ScratchImage &o_image)
	{
		...
		HRESULT result = E_UNEXPECTED;
		{
			...
			if (extension == L"dds")
			{
				...
				if (FAILED(result))
				{
					Tools::AssetBuildLibrary::UtilityFunctions::OutputErrorMessage("DirectXTex couldn't load the DDS file", i_path);
				}
			}
			else if (extension == L"tga")
			{
				...
				if (FAILED(result))
				{
					Tools::AssetBuildLibrary::UtilityFunctions::OutputErrorMessage("DirectXTex couldn't load the TGA file", i_path);
				}
			}
			else
			{
				...
				if (FAILED(result))
				{
					Tools::AssetBuildLibrary::UtilityFunctions::OutputErrorMessage("WIC couldn't load the source image", i_path);
				}
			}
		}
	...
	bool WriteTextureToFile(const char *const i_path, const DirectX::ScratchImage &i_texture)
	{
		...
		{
			Tools::AssetBuildLibrary::UtilityFunctions::OutputErrorMessage("DirectXTex failed to save the built texture to disk", i_path);
			return false;
		}
	}
}