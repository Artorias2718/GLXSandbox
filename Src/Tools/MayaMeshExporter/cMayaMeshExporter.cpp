...
MStatus Tools::MayaMeshExporter::cMayaMeshExporter::writer(const MFileObject& i_file, const MString& i_options, FileAccessMode i_mode)
{
	MStatus status;
...
namespace
{
		...
		const std::vector<sMaterialInfo>& i_materialInfo)
	{
		...
		if (fout.is_open())
		{
			...
			fout <<	"{\n";
			{
				fout << "\t--Vertex Count: " << i_vertexBuffer.size() << "\n";
				fout << "\tvertices = \n";
				fout << "\t{\n";
				for (size_t vertexIdx = 0; vertexIdx < i_vertexBuffer.size(); ++vertexIdx)
				{
					fout << "\t\t-- Vertex " << vertexIdx << "\n";
					fout << "\t\t{\n";
					fout << "\t\t\ttexture = { "
						<< "u = " << i_vertexBuffer[vertexIdx].vertex.u << ", "
						<< "v = " << i_vertexBuffer[vertexIdx].vertex.v <<
						" },\n";
					fout << "\t\t\tposition = { "
						<< "x = " << i_vertexBuffer[vertexIdx].vertex.x << ", "
						<< "y = " << i_vertexBuffer[vertexIdx].vertex.y << ", "
						<< "z = " << i_vertexBuffer[vertexIdx].vertex.z <<
						" },\n";
					fout << "\t\t\tcolor = { "
						<< "r = " << i_vertexBuffer[vertexIdx].vertex.r << ", "
						<< "g = " << i_vertexBuffer[vertexIdx].vertex.g << ", "
						<< "b = " << i_vertexBuffer[vertexIdx].vertex.b << ", "
						<< "a = " << i_vertexBuffer[vertexIdx].vertex.a <<
						" },\n";
					fout << "\t\t\tnormal = { "
						<< "x = " << i_vertexBuffer[vertexIdx].vertex.nx << ", "
						<< "y = " << i_vertexBuffer[vertexIdx].vertex.ny << ", "
						<< "z = " << i_vertexBuffer[vertexIdx].vertex.nz <<
						" },\n";
					fout << "\t\t\ttangent = { "
						<< "x = " << i_vertexBuffer[vertexIdx].vertex.tx << ", "
						<< "y = " << i_vertexBuffer[vertexIdx].vertex.ty << ", "
						<< "z = " << i_vertexBuffer[vertexIdx].vertex.tz <<
						" },\n";
					fout << "\t\t\tbitangent = { "
						<< "x = " << i_vertexBuffer[vertexIdx].vertex.btx << ", "
						<< "y = " << i_vertexBuffer[vertexIdx].vertex.bty << ", "
						<< "z = " << i_vertexBuffer[vertexIdx].vertex.btz <<
						" }\n";
					fout << "\t\t}";

					if (vertexIdx < i_vertexBuffer.size() - 1)
					{
						fout << ",";
					}
					fout << "\n";
				...
				fout << "\t},\n\n";
				fout << "\t--Triangle Count: " << i_indexBuffer.size() / indicesPerTriangle << "\n";
				fout << "\tindices = \n";
				fout << "\t{\n";
				for (size_t indexSetIdx = 0; indexSetIdx < i_indexBuffer.size();)
				{
					fout << "--Triangle " << triangleIdx++ << "\n";
					fout << "\t\t{ ";
					for (int indexIdx = 0; indexIdx < indicesPerTriangle; indexIdx++)
					{
						switch (indexIdx)
						{
						case 0:
							fout << "a = " << i_indexBuffer[indexSetIdx++] << ", ";
							break;
						case 1:
							fout << "b = " << i_indexBuffer[indexSetIdx++] << ", ";
							break;
						case 2:
							fout << "c = " << i_indexBuffer[indexSetIdx++];
							break;
						}
					}
					fout << " }";
					if (indexSetIdx < i_indexBuffer.size())
					{
						fout << ",\t\t";
					}
					else
