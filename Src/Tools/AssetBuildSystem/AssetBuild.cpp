...
bool Tools::AssetBuildSystem::AssetBuild::BuildAssets(const char* const i_path_assetsToBuild)
{
	// Load and execute the build script
	{
		...
		std::string path;
		{
			...
			if (!AssetBuildLibrary::UtilityFunctions::GetAssetBuildSystemPath(path, &errorMessage))
			{
				AssetBuildLibrary::UtilityFunctions::OutputErrorMessage(errorMessage.c_str(), __FILE__);
				return false;
			}
		}
		// Load and execute it
		{
			std::string errorMessage;
			if (Engine::Platform::DoesFileExist(path.c_str(), &errorMessage))
			{
			...
			{
				AssetBuildLibrary::UtilityFunctions::OutputErrorMessage(errorMessage.c_str(), path.c_str());
				return false;
			}
		}
	}
...
bool Tools::AssetBuildSystem::AssetBuild::Initialize()
{
	// Create a new Lua state
	{
		...
		if (!s_luaState)
		{
			AssetBuildLibrary::UtilityFunctions::OutputErrorMessage("Memory allocation error creating Lua state", __FILE__);
			return false;
		}
	}
...
bool Tools::AssetBuildSystem::AssetBuild::CleanUp()
{
...
namespace
{
	...
	int luaOutputErrorMessage(lua_State* io_luaState)
	{
		...
		Tools::AssetBuildLibrary::UtilityFunctions::OutputErrorMessage(i_errorMessage, i_optionalFileName);
