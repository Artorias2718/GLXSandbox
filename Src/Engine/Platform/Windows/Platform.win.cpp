...
{
	return Windows::Functions::CopyFile( i_path_source, i_path_target,
...
{
	return Windows::Functions::CreateDirectoryIfNecessary( i_path, o_errorMessage );
}

bool Engine::Platform::DoesFileExist( const char* const i_path, std::string* const o_errorMessage )
{
	return Windows::Functions::DoesFileExist( i_path, o_errorMessage );
}
...
bool Engine::Platform::ExecuteCommand( const char* const i_command, int* const o_exitCode, std::string* const o_errorMessage )
{
	...
	const bool result = Windows::Functions::ExecuteCommand( i_command, &exitCode_unsigned, o_errorMessage );
	if ( o_exitCode )
	{
...
{
	return Windows::Functions::GetEnvironmentVariable( i_key, o_value, o_errorMessage );
}

bool Engine::Platform::GetLastWriteTime( const char* const i_path, uint64_t& o_lastWriteTime, std::string* const o_errorMessage )
{
	return Windows::Functions::GetLastWriteTime( i_path, o_lastWriteTime, o_errorMessage );
}

bool Engine::Platform::InvalidateLastWriteTime( const char* const i_path, std::string* const o_errorMessage )
{
	return Windows::Functions::InvalidateLastWriteTime( i_path, o_errorMessage );
}
...
bool Engine::Platform::LoadBinaryFile( const char* const i_path, sDataFromFile& o_data, std::string* const o_errorMessage )
{
	...
	const bool result = Windows::Functions::LoadBinaryFile( i_path, dataFromFile, o_errorMessage );
	{
...
{
	return Windows::Functions::WriteBinaryFile( i_path, i_data, i_size, o_errorMessage );
}
