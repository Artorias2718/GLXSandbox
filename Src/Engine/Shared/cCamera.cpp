...
Engine::Shared::cCamera::cCamera(const std::string& i_fileName)
{
	...
	if (!i_fileName.empty())
	{
		...
		lua_gettable(luaState, -2);
		m_fieldOfView = glm::radians(static_cast<float>(lua_tonumber(luaState, -1)));
		lua_pop(luaState, 1);
...

		{
			...
			angle = glm::radians(static_cast<float>(lua_tonumber(luaState, -1)));
			lua_pop(luaState, 1);
			...
			m_transform.orientation = glm::quat(angle, axis);
		}
...
{
	m_worldToCamera = Engine::Math::CreateWorldToCameraTransform(m_transform);
	m_cameraToScreen = Engine::Math::CreateCameraToScreenTransform(
		m_fieldOfView, m_aspectRatio, m_nearPlane, m_farPlane);

	return true;
}

glm::mat4x4 Engine::Shared::cCamera::WorldToCamera()
{
	return m_worldToCamera;
}

glm::mat4x4 Engine::Shared::cCamera::CameraToScreen()
{
