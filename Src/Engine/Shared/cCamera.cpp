...
Engine::Shared::cCamera::cCamera(const std::string& i_fileName)
{
	...
	if (!i_fileName.empty())
	{
		...
		lua_gettable(luaState, -2);
		m_fieldOfView = Math::ConvertDegreesToRadians(static_cast<float>(lua_tonumber(luaState, -1)));
		lua_pop(luaState, 1);
...

		{
			...
			angle = Math::ConvertDegreesToRadians(static_cast<float>(lua_tonumber(luaState, -1)));
			lua_pop(luaState, 1);
			...
			m_transform.orientation = Math::cQuaternion(angle, axis);
		}
...
{
	Math::cQuaternion rotation(Math::ConvertDegreesToRadians(m_rotationSpeed), Shared::MouseParams::verticalAxis);
...
{
	m_worldToCamera = Math::cMatrix_Transformation::CreateWorldToCameraTransform(m_transform);
	m_cameraToScreen = Math::cMatrix_Transformation::CreateCameraToScreenTransform(
		m_fieldOfView, m_aspectRatio, m_nearPlane, m_farPlane);

	return true;
}

Engine::Math::cMatrix_Transformation Engine::Shared::cCamera::WorldToCamera()
{
	return m_worldToCamera;
}

Engine::Math::cMatrix_Transformation Engine::Shared::cCamera::CameraToScreen()
{
