...
void Engine::Graphics::Interfaces::cRenderState::Bind() const
{
	// Alpha Transparency
	{
		...
		D3DInterfaces::s_direct3dImmediateContext->OMSetBlendState(m_blendState, noBlendFactor, defaultSampleMask);
	}
	// Depth Buffering
	{
		ASSERT(m_depthStencilState != NULL);
		const unsigned int unusedStencilReference = 0;
		D3DInterfaces::s_direct3dImmediateContext->OMSetDepthStencilState(m_depthStencilState, unusedStencilReference);
	}
	// Cull Backfacing Triangles
	{
		ASSERT(m_rasterizerState != NULL);
		D3DInterfaces::s_direct3dImmediateContext->RSSetState(m_rasterizerState);
	}
}
...
bool Engine::Graphics::Interfaces::cRenderState::InitializeFromBits()
{
	...
	// Alpha Transparency
	{
		...
		const HRESULT result = D3DInterfaces::s_direct3dDevice->CreateBlendState(&blendStateDescription, &m_blendState);
		if (FAILED(result))
		{
			wereThereErrors = true;
			ASSERTF(false, "CreateBlendState() failed");
			Logging::OutputError("Direct3D failed to create a blend render state object from %u with HRESULT %#010x", m_bits, result);
			goto OnExit;
		}
	}
	// Depth Buffering
	{
		...
		const HRESULT result = D3DInterfaces::s_direct3dDevice->CreateDepthStencilState(&depthStateDescription, &m_depthStencilState);
		if (FAILED(result))
		{
			wereThereErrors = true;
			ASSERTF(false, "CreateDepthStencilState() failed");
			Logging::OutputError("Direct3D failed to create a depth/stencil render state object from %u with HRESULT %#010x", m_bits, result);
			goto OnExit;
		}
	}
	// Draw Both Triangle Sides
	{
		...
		const HRESULT result = D3DInterfaces::s_direct3dDevice->CreateRasterizerState(&rasterizerStateDescription, &m_rasterizerState);
		if (FAILED(result))
		{
			wereThereErrors = true;
			ASSERTF(false, "CreateRasterizerState() failed");
			Logging::OutputError("Direct3D failed to create a rasterizer render state object from %u with HRESULT %#010x", m_bits, result);
			goto OnExit;
		}
	}
