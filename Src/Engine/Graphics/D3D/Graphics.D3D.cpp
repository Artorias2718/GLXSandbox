...
namespace
{
	...
	ID3D11Device* s_D3DDevice = NULL;
	IDXGISwapChain* s_swapChain = NULL;
	ID3D11DeviceContext* s_D3DImmediateContext = NULL;
...
void Engine::Graphics::RenderFrame()
{
	...
	// by "clearing" the image buffer (filling it with a solid color)
	{
		...
		s_D3DImmediateContext->ClearRenderTargetView(s_renderTargetView, clearColor);
	}
	...
	// Update the constant buffer
	{
		...
		// Get a pointer from D3D that can be written to
		void* memoryToWriteTo = NULL;
		{
			D3D11_MAPPED_SUBRESOURCE mappedSubResource;
			{
				...
				const HRESULT result = s_D3DImmediateContext->Map(s_constantBuffer, noSubResources, mapType, noFlags, &mappedSubResource);
				if (SUCCEEDED(result))
				{
		...
		if (memoryToWriteTo)
		{
			// Copy the new data to the memory that D3D has provided
			memcpy(memoryToWriteTo, &s_constantBufferData, sizeof(s_constantBufferData));
			// Let D3D know that the memory contains the data
			// (the pointer will be invalid after this call)
			const unsigned int noSubResources = 0;
			s_D3DImmediateContext->Unmap(s_constantBuffer, noSubResources);
		...
		// Bind the constant buffer to the shader
		{
			...
			s_D3DImmediateContext->VSSetConstantBuffers(registerAssignedInShader, bufferCount, &s_constantBuffer);
			s_D3DImmediateContext->PSSetConstantBuffers(registerAssignedInShader, bufferCount, &s_constantBuffer);
		}
	}
	...
	// Draw the geometry
	{
		// Set the vertex and fragment shaders
		{
			...
			s_D3DImmediateContext->VSSetShader(s_vertexShader, noInterfaces, interfaceCount);
			s_D3DImmediateContext->PSSetShader(s_fragmentShader, noInterfaces, interfaceCount);
		}
		// Bind a specific vertex buffer to the device as a data source
		{
			...
			s_D3DImmediateContext->IASetVertexBuffers(startingSlot, vertexBufferCount, &s_vertexBuffer, &bufferStride, &bufferOffset);
		}
		// Specify what kind of data the vertex buffer holds
		{
			// Set the layout (which defines how to interpret a single vertex)
			s_D3DImmediateContext->IASetInputLayout(s_vertexLayout);
			// Set the topology (which defines how to interpret multiple vertices as a single "primitive";
			// we have defined the vertex buffer as a triangle list
			// (meaning that every primitive is a triangle and will be defined by three vertices)
			s_D3DImmediateContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
		}
		// Render triangles from the currently-bound vertex buffer
		{
			...
			s_D3DImmediateContext->Draw(vertexCountToRender, indexOfFirstVertexToRender);
		}
	}
...
bool Engine::Graphics::Initialize(const sInitializationParameters& i_initializationParameters)
{
	...
	// Create an interface to a D3D device
...
bool Engine::Graphics::CleanUp()
{
	...
	if (s_D3DImmediateContext)
	{
		s_D3DImmediateContext->Release();
		s_D3DImmediateContext = NULL;
	}
	if (s_swapChain)
	{
		s_swapChain->Release();
...
namespace
{
	bool CreateConstantBuffer()
	{
		...
		const HRESULT result = s_D3DDevice->CreateBuffer(&bufferDescription, &initialData, &s_constantBuffer);
		if (SUCCEEDED(result))
		{
			return true;
		}
		...
		{
			ASSERT(false);
			Engine::Logging::OutputError("D3D failed to create a constant buffer with HRESULT %#010x", result);
			return false;
		}
	}
	...
	bool CreateDevice(const unsigned int i_resolutionWidth, const unsigned int i_resolutionHeight)
	{
		...
		const HRESULT result = D3D11CreateDeviceAndSwapChain(useDefaultAdapter, useHardwareRendering, dontUseSoftwareRendering,
			...
			&s_swapChain, &s_D3DDevice, &highestSupportedFeatureLevel, &s_D3DImmediateContext);
		if (SUCCEEDED(result))
		{
			return true;
		}
		...
		{
			ASSERT(false);
			Engine::Logging::OutputError("D3D failed to create a D3D11 device with HRESULT %#010x", result);
			return false;
		}
	}
	...
	bool CreateVertexBuffer(ID3D10Blob& i_compiledShader)
	{
		// Create the vertex layout
		{
			...
			// They instruct D3D how to match the binary data in the vertex buffer
			// to the input elements in a vertex shader
			// (by using so-called "semantic" names so that, for example,
			// "POSITION" here matches with "POSITION" in shader code).
			// Note that OGL uses arbitrarily assignable number IDs to do the same thing.
			...
			const HRESULT result = s_D3DDevice->CreateInputLayout(layoutDescription, vertexElementCount,
				i_compiledShader.GetBufferPointer(), i_compiledShader.GetBufferSize(), &s_vertexLayout);
			if (FAILED(result))
			{
				ASSERT(false);
				Engine::Logging::OutputError("D3D failed to create a vertex input layout with HRESULT %#010x", result);
				return false;
			}
		}
		...
		const HRESULT result = s_D3DDevice->CreateBuffer(&bufferDescription, &initialData, &s_vertexBuffer);
		if (FAILED(result))
		{
			ASSERT(false);
			Engine::Logging::OutputError("D3D failed to create the vertex buffer with HRESULT %#010x", result);
			return false;
		}
	...
	bool CreateView(const unsigned int i_resolutionWidth, const unsigned int i_resolutionHeight)
	{
		...
		ID3D11Texture2D* backBuffer = NULL;
		{
			...
			HRESULT result;
			{
				...
				{
					ASSERT(false);
					Engine::Logging::OutputError("D3D failed to get the back buffer from the swap chain with HRESULT %#010x", result);
					goto OnExit;
				}
			}
			// Create the view
			{
				...
				result = s_D3DDevice->CreateRenderTargetView(backBuffer, accessAllSubResources, &s_renderTargetView);
			}
			if (SUCCEEDED(result))
			{
				// Bind it
				...
				s_D3DImmediateContext->OMSetRenderTargets(renderTargetCount, &s_renderTargetView, noDepthStencilState);
			}
			else
			{
				ASSERT(false);
				Engine::Logging::OutputError("D3D failed to create the render target view with HRESULT %#010x", result);
				goto OnExit;
			}
		}
		...
		// Specify that the entire render target should be visible
		{
			...
			s_D3DImmediateContext->RSSetViewports(viewPortCount, &viewPort);
		}
	...
	bool LoadFragmentShader()
	{
		...
		{
			const char* const sourceCodeFileName = "data/shaders/d3d/fragment.hlsl";
			...
			else
			{
				if (errorMessages)
				{
					...
					Engine::Logging::OutputError("D3D failed to compile the fragment shader from the file %s: %s",
				...
				{
					ASSERT(false);
					Engine::Logging::OutputError("D3D failed to compile the fragment shader from the file %s",
						sourceCodeFileName);
				}
				return false;
			}
		...
		bool wereThereErrors = false;
		{
			...
			const HRESULT result = s_D3DDevice->CreatePixelShader(compiledShader->GetBufferPointer(), compiledShader->GetBufferSize(),
				noInterfaces, &s_fragmentShader);
			if (FAILED(result))
			{
				ASSERT(false);
				Engine::Logging::OutputError("D3D failed to create the fragment shader with HRESULT %#010x", result);
				wereThereErrors = true;
			}
	...
	bool LoadVertexShader(ID3D10Blob*& o_compiledShader)
	{
		// Load the source code and compile it
		{
			const char* const sourceCodeFileName = "data/shaders/d3d/vertex.hlsl";
			D3D10_SHADER_MACRO* const noMacros = NULL;
			ID3DInclude* const noIncludes = NULL;
			const char* const entryPoint = "main";
			...
			else
			{
				...
				{
					ASSERTF(false, reinterpret_cast<char*>(errorMessages->GetBufferPointer()));
					Engine::Logging::OutputError("D3D failed to compile the vertex shader from the file %s: %s",
						sourceCodeFileName, reinterpret_cast<char*>(errorMessages->GetBufferPointer()));
					errorMessages->Release();
				}
				else
				{
					ASSERT(false);
					Engine::Logging::OutputError("D3D failed to compile the vertex shader from the file %s",
						sourceCodeFileName);
				}
				return false;
			}
		...
		{
			ID3D11ClassLinkage* const noInterfaces = NULL;
			const HRESULT result = s_D3DDevice->CreateVertexShader(o_compiledShader->GetBufferPointer(), o_compiledShader->GetBufferSize(),
				noInterfaces, &s_vertexShader);
			if (FAILED(result))
			{
				ASSERT(false);
				Engine::Logging::OutputError("D3D failed to create the vertex shader with HRESULT %#010x", result);
				wereThereErrors = true;
			}
		}
		return !wereThereErrors;
