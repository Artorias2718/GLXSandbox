...
Engine::Graphics::Assets::Debug::cSphere::cSphere()
{
	...
	AddVertex(Math::cVector(-x, z, n));
	AddVertex(Math::cVector(x, z, n));
	AddVertex(Math::cVector(-x, -z, n));
	AddVertex(Math::cVector(x, -z, n));

	AddVertex(Math::cVector(n, -x, z));
	AddVertex(Math::cVector(n, x, z));
	AddVertex(Math::cVector(n, -x, -z));
	AddVertex(Math::cVector(n, x, -z));

	AddVertex(Math::cVector(z, n, -x));
	AddVertex(Math::cVector(z, n, x));
	AddVertex(Math::cVector(-z, n, -x));
	AddVertex(Math::cVector(-z, n, x));
...
uint16_t Engine::Graphics::Assets::Debug::cSphere::MidPointIdx(uint16_t i_idx1, uint16_t i_idx2)
{
	...
	Math::cVector midpoint = 0.5f * Math::cVector(point1.position.x + point2.position.x, point1.position.y + point2.position.y, point1.position.z + point2.position.z);
