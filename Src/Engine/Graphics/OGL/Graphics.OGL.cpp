...
namespace
{
	...
	HGLRC s_OGLRenderingContext = NULL;

	// The vertex buffer holds the data for each vertex
	GLuint s_vertexArrayId = 0;
#ifdef GRAPHICS_ISDEVICEDEBUGINFOENABLED
	// OGL debuggers don't seem to support freeing the vertex buffer
	...
	// OGL encapsulates a matching vertex shader and fragment shader into what it calls a "program".
...
bool Engine::Graphics::Initialize(const sInitializationParameters& i_initializationParameters)
{
	...
	// Load any required OGL extensions
	...
	// Create an OGL rendering context
	...
	if (s_OGLRenderingContext != NULL)
	{
		if (s_programId != 0)
		{
			...
			if (errorCode != GL_NO_ERROR)
			{
				...
				Logging::OutputError("OGL failed to delete the program: %s",
		...
		if (s_vertexBufferId != 0)
		{
			...
			if (errorCode != GL_NO_ERROR)
			{
				wereThereErrors = true;
				ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
				Logging::OutputError("OGL failed to delete the vertex buffer: %s",
					reinterpret_cast<const char*>(gluErrorString(errorCode)));
			}
			s_vertexBufferId = 0;
		}
		...
		if (s_vertexArrayId != 0)
		{
			...
			if (errorCode != GL_NO_ERROR)
			{
				wereThereErrors = true;
				ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
				Logging::OutputError("OGL failed to delete the vertex array: %s",
					reinterpret_cast<const char*>(gluErrorString(errorCode)));
			}
			s_vertexArrayId = 0;
		}
		...
		if (s_constantBufferId != 0)
		{
			...
			if (errorCode != GL_NO_ERROR)
			{
				wereThereErrors = true;
				ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
				Logging::OutputError("OGL failed to delete the constant buffer: %s",
					reinterpret_cast<const char*>(gluErrorString(errorCode)));
			}
		...
		{
			if (wglDeleteContext(s_OGLRenderingContext) == FALSE)
			{
				wereThereErrors = true;
				const std::string windowsErrorMessage = Windows::GetLastSystemError();
				ASSERTF(false, windowsErrorMessage.c_str());
				Logging::OutputError("Windows failed to delete the OGL rendering context: %s", windowsErrorMessage.c_str());
			}
		}
		else
		{
			...
			const std::string windowsErrorMessage = Windows::GetLastSystemError();
			ASSERTF(false, windowsErrorMessage.c_str());
			Logging::OutputError("Windows failed to unset the current OGL rendering context: %s", windowsErrorMessage.c_str());
		}
		s_OGLRenderingContext = NULL;
	}
...
namespace
{
	bool CreateConstantBuffer()
	{
		...
		// Create a uniform buffer object and make it active
		{
			...
			if (errorCode == GL_NO_ERROR)
			{
				...
				if (errorCode != GL_NO_ERROR)
				{
					wereThereErrors = true;
					ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
					Engine::Logging::OutputError("OGL failed to bind the new uniform buffer %u: %s",
			...
			else
			{
				wereThereErrors = true;
				ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
				Engine::Logging::OutputError("OGL failed to get an unused uniform buffer ID: %s",
					reinterpret_cast<const char*>(gluErrorString(errorCode)));
				goto OnExit;
			}
		}
		...
		// Allocate space and copy the constant data into the uniform buffer
		{
			...
			if (errorCode != GL_NO_ERROR)
			{
				wereThereErrors = true;
				ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
				Engine::Logging::OutputError("OGL failed to allocate the new uniform buffer %u: %s",
					s_constantBufferId, reinterpret_cast<const char*>(gluErrorString(errorCode)));
				goto OnExit;
			}
		}
	...
	bool CreateProgram()
	{
		// Create a program
		{
			...
			{
				ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
				Engine::Logging::OutputError("OGL failed to create a program: %s",
					reinterpret_cast<const char*>(gluErrorString(errorCode)));
				return false;
			}
			...
			{
				ASSERT(false);
				Engine::Logging::OutputError("OGL failed to create a program");
				return false;
			}
		}
		...
		// Link the program
		{
			...
			if (errorCode == GL_NO_ERROR)
			{
				...
				std::string linkInfo;
				{
					...
					if (errorCode == GL_NO_ERROR)
					{
						...
						{
							ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
							Engine::Logging::OutputError("OGL failed to get link info of the program: %s",
								reinterpret_cast<const char*>(gluErrorString(errorCode)));
							return false;
						}
					}
					...
					{
						ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
						Engine::Logging::OutputError("OGL failed to get the length of the program link info: %s",
							reinterpret_cast<const char*>(gluErrorString(errorCode)));
						return false;
					}
				}
				...
				GLint didLinkingSucceed;
				{
					...
					{
						ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
						Engine::Logging::OutputError("OGL failed to find out if linking of the program succeeded: %s",
							reinterpret_cast<const char*>(gluErrorString(errorCode)));
						return false;
					}
				}
			...
			{
				ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
				Engine::Logging::OutputError("OGL failed to link the program: %s",
					reinterpret_cast<const char*>(gluErrorString(errorCode)));
				return false;
			}
		}
	...
	bool CreateRenderingContext()
	{
		...
		// Create an OGL rendering context and make it current
		{
			// Create the context
			{
				...
				s_OGLRenderingContext = wglCreateContextAttribsARB(s_deviceContext, noSharedContexts, desiredAttributes);
				if (s_OGLRenderingContext == NULL)
				{
					DWORD errorCode;
					const std::string windowsErrorMessage = Engine::Windows::GetLastSystemError(&errorCode);
					std::ostringstream errorMessage;
					errorMessage << "Windows failed to create an OGL rendering context: ";
			...
			if (wglMakeCurrent(s_deviceContext, s_OGLRenderingContext) == FALSE)
			{
				const std::string windowsErrorMessage = Engine::Windows::GetLastSystemError();
				ASSERTF(false, windowsErrorMessage.c_str());
				Engine::Logging::OutputError("Windows failed to set the current OGL rendering context: %s",
	...
	bool CreateVertexBuffer()
	{
		...
		// Create a vertex array object and make it active
		{
			...
			if (errorCode == GL_NO_ERROR)
			{
				...
				if (errorCode != GL_NO_ERROR)
				{
					wereThereErrors = true;
					ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
					Engine::Logging::OutputError("OGL failed to bind the vertex array: %s",
						reinterpret_cast<const char*>(gluErrorString(errorCode)));
					goto OnExit;
				}
			}
			else
			{
				wereThereErrors = true;
				ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
				Engine::Logging::OutputError("OGL failed to get an unused vertex array ID: %s",
					reinterpret_cast<const char*>(gluErrorString(errorCode)));
				goto OnExit;
			}
		}
		...
		// Create a vertex buffer object and make it active
		{
			...
			if (errorCode == GL_NO_ERROR)
			{
				...
				if (errorCode != GL_NO_ERROR)
				{
					wereThereErrors = true;
					ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
					Engine::Logging::OutputError("OGL failed to bind the vertex buffer: %s",
						reinterpret_cast<const char*>(gluErrorString(errorCode)));
					goto OnExit;
				}
			}
			else
			{
				wereThereErrors = true;
				ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
				Engine::Logging::OutputError("OGL failed to get an unused vertex buffer ID: %s",
					reinterpret_cast<const char*>(gluErrorString(errorCode)));
				goto OnExit;
			}
		}
		// Assign the data to the buffer
		{
			...
			if (errorCode != GL_NO_ERROR)
			{
				wereThereErrors = true;
				ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
				Engine::Logging::OutputError("OGL failed to allocate the vertex buffer: %s",
					reinterpret_cast<const char*>(gluErrorString(errorCode)));
				goto OnExit;
			}
		}
			...
			// Offset = 0
			{
				...
				if (errorCode == GL_NO_ERROR)
				{
					...
					if (errorCode != GL_NO_ERROR)
					{
						wereThereErrors = true;
						ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
						Engine::Logging::OutputError("OGL failed to enable the POSITION vertex attribute at location %u: %s",
				...
				else
				{
					wereThereErrors = true;
					ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
					Engine::Logging::OutputError("OGL failed to set the POSITION vertex attribute at location %u: %s",
						vertexElementLocation, reinterpret_cast<const char*>(gluErrorString(errorCode)));
					goto OnExit;
				}
			}
	...
	OnExit:
		...
		if (s_vertexArrayId != 0)
		{
			...
			if (errorCode == GL_NO_ERROR)
			{
				...
				if (vertexBufferId != 0)
				{
					...
					if (errorCode != GL_NO_ERROR)
					{
						wereThereErrors = true;
						ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
						Engine::Logging::OutputError("OGL failed to vertex buffer: %s",
							reinterpret_cast<const char*>(gluErrorString(errorCode)));
						goto OnExit;
					}
					vertexBufferId = 0;
			...
			else
			{
				wereThereErrors = true;
				ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
				Engine::Logging::OutputError("OGL failed to unbind the vertex array: %s",
					reinterpret_cast<const char*>(gluErrorString(errorCode)));
				goto OnExit;
			}
		}
	...
	bool LoadFragmentShader(const GLuint i_programId)
	{
		...
		Engine::Platform::sDataFromFile dataFromFile;
		{
			...
			fragmentShaderId = glCreateShader(GL_FRAGMENT_SHADER);
			{
				...
				if (errorCode != GL_NO_ERROR)
				{
					wereThereErrors = true;
					ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
					Engine::Logging::OutputError("OGL failed to get an unused fragment shader ID: %s",
						reinterpret_cast<const char*>(gluErrorString(errorCode)));
					goto OnExit;
				}
				else if (fragmentShaderId == 0)
				{
					wereThereErrors = true;
					ASSERT(false);
					Engine::Logging::OutputError("OGL failed to get an unused fragment shader ID");
					goto OnExit;
				}
			}
			// Set the source code into the shader
			{
				...
				if (errorCode != GL_NO_ERROR)
				{
					wereThereErrors = true;
					ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
					Engine::Logging::OutputError("OGL failed to set the fragment shader source code: %s",
						reinterpret_cast<const char*>(gluErrorString(errorCode)));
					goto OnExit;
				}
			}
		...
		// Compile the shader source code
		{
				...
				std::string compilationInfo;
				{
					...
					if (errorCode == GL_NO_ERROR)
					{
						...
						else
						{
							wereThereErrors = true;
							ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
							Engine::Logging::OutputError("OGL failed to get compilation info about the fragment shader source code: %s",
								reinterpret_cast<const char*>(gluErrorString(errorCode)));
							goto OnExit;
						}
					}
					else
					{
						wereThereErrors = true;
						ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
						Engine::Logging::OutputError("OGL failed to get the length of the fragment shader compilation info: %s",
							reinterpret_cast<const char*>(gluErrorString(errorCode)));
						goto OnExit;
					}
				}
				...
				GLint didCompilationSucceed;
				{
					...
					if (errorCode == GL_NO_ERROR)
					{
						if (didCompilationSucceed == GL_FALSE)
						{
							...
							Engine::Logging::OutputError("OGL failed to compile the fragment shader: %s",
					...
					else
					{
						wereThereErrors = true;
						ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
						Engine::Logging::OutputError("OGL failed to find if compilation of the fragment shader source code succeeded: %s",
							reinterpret_cast<const char*>(gluErrorString(errorCode)));
						goto OnExit;
					}
				}
			...
			else
			{
				wereThereErrors = true;
				ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
				Engine::Logging::OutputError("OGL failed to compile the fragment shader source code: %s",
					reinterpret_cast<const char*>(gluErrorString(errorCode)));
				goto OnExit;
			}
		}
		// Attach the shader to the program
		{
			...
			if (errorCode != GL_NO_ERROR)
			{
				wereThereErrors = true;
				ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
				Engine::Logging::OutputError("OGL failed to attach the fragment to the program: %s",
					reinterpret_cast<const char*>(gluErrorString(errorCode)));
				goto OnExit;
			}
		}
	...
	OnExit:
		...
		if (fragmentShaderId != 0)
		{
			...
			// (any associated memory that OGL has allocated internally will be freed
			...
			if (errorCode != GL_NO_ERROR)
			{
				wereThereErrors = true;
				ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
				Engine::Logging::OutputError("OGL failed to delete the fragment shader ID %u: %s",
	...
	bool LoadVertexShader(const GLuint i_programId)
	{
		...
		Engine::Platform::sDataFromFile dataFromFile;
		{
			...
			vertexShaderId = glCreateShader(GL_VERTEX_SHADER);
			{
				...
				if (errorCode != GL_NO_ERROR)
				{
					wereThereErrors = true;
					ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
					Engine::Logging::OutputError("OGL failed to get an unused vertex shader ID: %s",
						reinterpret_cast<const char*>(gluErrorString(errorCode)));
					goto OnExit;
				}
				else if (vertexShaderId == 0)
				{
					wereThereErrors = true;
					ASSERT(false);
					Engine::Logging::OutputError("OGL failed to get an unused vertex shader ID");
					goto OnExit;
				}
			}
			// Set the source code into the shader
			{
				...
				if (errorCode != GL_NO_ERROR)
				{
					wereThereErrors = true;
					ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
					Engine::Logging::OutputError("OGL failed to set the vertex shader source code: %s",
						reinterpret_cast<const char*>(gluErrorString(errorCode)));
					goto OnExit;
				}
			}
		...
		// Compile the shader source code
		{
				...
				std::string compilationInfo;
				{
					...
					if (errorCode == GL_NO_ERROR)
					{
						...
						else
						{
							wereThereErrors = true;
							ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
							Engine::Logging::OutputError("OGL failed to get compilation info about the vertex shader source code: %s",
								reinterpret_cast<const char*>(gluErrorString(errorCode)));
							goto OnExit;
						}
					}
					else
					{
						wereThereErrors = true;
						ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
						Engine::Logging::OutputError("OGL failed to get the length of the vertex shader compilation info: %s",
							reinterpret_cast<const char*>(gluErrorString(errorCode)));
						goto OnExit;
					}
				}
				...
				GLint didCompilationSucceed;
				{
					...
					if (errorCode == GL_NO_ERROR)
					{
						if (didCompilationSucceed == GL_FALSE)
						{
							wereThereErrors = true;
							ASSERTF(false, compilationInfo.c_str());
							Engine::Logging::OutputError("OGL failed to compile the vertex shader: %s",
								compilationInfo.c_str());
							goto OnExit;
						}
					}
					else
					{
						wereThereErrors = true;
						ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
						Engine::Logging::OutputError("OGL failed to find if compilation of the vertex shader source code succeeded: %s",
							reinterpret_cast<const char*>(gluErrorString(errorCode)));
						goto OnExit;
					}
				}
			...
			else
			{
				wereThereErrors = true;
				ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
				Engine::Logging::OutputError("OGL failed to compile the vertex shader source code: %s",
					reinterpret_cast<const char*>(gluErrorString(errorCode)));
				goto OnExit;
			}
		}
		// Attach the shader to the program
		{
			...
			if (errorCode != GL_NO_ERROR)
			{
				wereThereErrors = true;
				ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
				Engine::Logging::OutputError("OGL failed to attach the vertex to the program: %s",
					reinterpret_cast<const char*>(gluErrorString(errorCode)));
				goto OnExit;
			}
		}
	...
	OnExit:
		...
		if (vertexShaderId != 0)
		{
			...
			if (errorCode != GL_NO_ERROR)
			{
				wereThereErrors = true;
				ASSERTF(false, reinterpret_cast<const char*>(gluErrorString(errorCode)));
				Engine::Logging::OutputError("OGL failed to delete the vertex shader ID %u: %s",
