...
namespace
{
	const char* const s_hiddenWindowClass_name = "EAE6320 Hidden OGL Context Window Class";
}
...
bool Engine::Windows::OGL::CreateHiddenContextWindow( HINSTANCE& io_hInstance, sHiddenWindowInfo& o_info, std::string* const o_errorMessage )
{
	...
	// Create the hidden window
	{
		...
		// Register the hidden window's class
		{
			...
			if ( o_info.windowClass == NULL )
			{
				...
				if ( o_errorMessage )
				{
					const std::string windowsErrorMessage = GetLastSystemError();
					std::ostringstream errorMessage;
					errorMessage << "Windows failed to register the hidden OGL context window's class: " << windowsErrorMessage;
					*o_errorMessage = errorMessage.str();
				}
				goto OnExit;
			}
		...
		// Create the hidden window
		{
			const char* const windowName = "EAE6320 Hidden OGL Context Window";
			...
			if ( o_info.window == NULL )
			{
				...
				if ( o_errorMessage )
				{
					const std::string windowsErrorMessage = GetLastSystemError();
					std::ostringstream errorMessage;
					errorMessage << "Windows failed to create the hidden OGL context window: " << windowsErrorMessage;
					*o_errorMessage = errorMessage.str();
				}
				goto OnExit;
			}
	...
	// Get the device context
	{
		...
		if ( o_info.deviceContext == NULL )
		{
			...
			if ( o_errorMessage )
			{
				*o_errorMessage = "Windows failed to get the hidden OGL device context";
			}
			goto OnExit;
		}
	}
			...
			if ( pixelFormatId == 0 )
			{
				...
				if ( o_errorMessage )
				{
					...
					errorMessage << "Windows couldn't choose the closest pixel format"
						" for the hidden OGL context window: "
						<< windowsErrorMessage;
					*o_errorMessage = errorMessage.str();
				}
				goto OnExit;
	...
	if ( o_info.OGLRenderingContext == NULL )
	{
		...
		if ( o_errorMessage )
		{
			const std::string windowsErrorMessage = GetLastSystemError();
			std::ostringstream errorMessage;
			errorMessage << "Windows failed to create a hidden OGL rendering context: "
				<< windowsErrorMessage;
			*o_errorMessage = errorMessage.str();
		}
		goto OnExit;
	...
	if ( wglMakeCurrent( o_info.deviceContext, o_info.OGLRenderingContext ) == FALSE )
	{
		...
		if ( o_errorMessage )
		{
			const std::string windowsErrorMessage = GetLastSystemError();
			std::ostringstream errorMessage;
			errorMessage << "Windows failed to set the current OGL rendering context"
	...
	if ( io_info.OGLRenderingContext != NULL )
	{
		...
		if ( wglGetCurrentContext() == io_info.OGLRenderingContext )
		{
			if ( wglMakeCurrent( io_info.deviceContext, NULL ) == FALSE )
			{
				...
				if ( o_errorMessage )
				{
					...
					std::ostringstream errorMessage;
					errorMessage << "\nWindows failed to unset the current hidden OGL device context: " << windowsError;
					*o_errorMessage += errorMessage.str();
				}
			}
		}
		if ( wglDeleteContext( io_info.OGLRenderingContext ) == FALSE )
		{
			...
			if ( o_errorMessage )
			{
				const std::string windowsError = GetLastSystemError();
				std::ostringstream errorMessage;
				errorMessage << "\nWindows failed to delete the hidden OGL rendering context: " << windowsError;
				*o_errorMessage += errorMessage.str();
			}
		}
	...
	if ( io_info.window != NULL )
	{
		...
		if ( DestroyWindow( io_info.window ) == FALSE )
		{
			...
			if ( o_errorMessage )
			{
				const std::string windowsErrorMessage = GetLastSystemError();
				std::ostringstream errorMessage;
				errorMessage << "\nWindows failed to destroy the hidden OGL context window: " << windowsErrorMessage;
				*o_errorMessage += errorMessage.str();
			}			
		}
		io_info.window = NULL;
	...
	if ( io_info.windowClass != NULL )
	{
		...
		if ( UnregisterClass( s_hiddenWindowClass_name, i_hInstance ) == FALSE )
		{
			...
			if ( o_errorMessage )
			{
				const std::string windowsErrorMessage = GetLastSystemError();
				std::ostringstream errorMessage;
				errorMessage << "\nWindows failed to unregister the hidden OGL context window's class \""
