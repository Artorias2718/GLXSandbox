...
namespace
{
	...
	bool CreateMainWindow( const HINSTANCE i_thisInstanceOfTheApplication, const char* const i_windowName, const ATOM i_windowClass,
		...
		Engine::Application::cBaseApplication& io_application, HWND& o_window );
...
bool Engine::Application::cBaseApplication::GetResolution( unsigned int& o_width, unsigned int& o_height ) const
{
...
Engine::Application::cBaseApplication* Engine::Application::cBaseApplication::GetApplicationFromWindow( const HWND i_window )
{
	const LONG_PTR userData = GetWindowLongPtr( i_window, GWLP_USERDATA );
	cBaseApplication* const application = reinterpret_cast<cBaseApplication*>( userData );
...
bool Engine::Application::cBaseApplication::WaitForApplicationToFinish( int& o_exitCode )
{
...
LRESULT CALLBACK Engine::Application::cBaseApplication::OnMessageReceivedFromWindows( HWND i_window, UINT i_message, WPARAM i_wParam, LPARAM i_lParam )
{
	...
	case WM_CREATE:
		{
			...
			Engine::Application::cBaseApplication* application =
				reinterpret_cast<Engine::Application::cBaseApplication*>( creationData.lpCreateParams );
			ASSERT( application != NULL );
	...
	case WM_NCDESTROY:
		{
			...
			{
				cBaseApplication* const application = GetApplicationFromWindow( i_window );
				if ( application != NULL )
				{
					application->m_mainWindow = NULL;
				}
	...
	// Append this function so ImGUI can send messages to Windows 
	return DefWindowProc( i_window, i_message, i_wParam, i_lParam )
		 | ImGui_ImplWin32_WndProcHandler(i_window, i_message, i_wParam, i_lParam);
}

// Initialization / Clean Up
//--------------------------

bool Engine::Application::cBaseApplication::InitializeBase( const sEntryPointParameters& i_entryPointParameters )
{
...
bool Engine::Application::cBaseApplication::PopulateGraphicsInitializationParameters( Graphics::sInitializationParameters& o_initializationParameters )
{
...
bool Engine::Application::cBaseApplication::PopulateUserOutputInitializationParameters( UserOutput::sInitializationParameters& o_initializationParameters )
{
	ASSERT( m_mainWindow != NULL );
	o_initializationParameters.mainWindow = m_mainWindow;
	return true;
...
bool Engine::Application::cBaseApplication::CleanUpBase()
{
	bool wereThereErrors = false;
...
Engine::Application::cBaseApplication::cBaseApplication()
	:
