...
do
	...
	-- that has AssetTypeInfo assigned as its metatable
	-- will do the following when indexed:
	--	* If the table itself has the key being indexed it will be used
	--	* Otherwise the key of AssetTypeInfo will be used
	AssetTypeInfo.__index = AssetTypeInfo
end
...
local function NewAssetTypeInfo( i_assetTypeInfoKey, io_newAssetTypeInfoTable )
	...
	if not assetTypeInfos[i_assetTypeInfoKey] then
		if type( io_newAssetTypeInfoTable ) == "table" then
			-- Setting the base class AssetTypeInfo as the metatable
			-- of the given table causes the given table to inherit from AssetTypeInfo
			setmetatable( io_newAssetTypeInfoTable, AssetTypeInfo )
...
local function RegisterAssetToBeBuilt( i_sourceAssetRelativePath, i_assetType, i_optionalCommandLineArguments )
	...
	do
		...
		elseif getmetatable( i_assetType ) == AssetTypeInfo then
...
function AssetTypeInfo.GetBuilderRelativePath()
...
function AssetTypeInfo.ConvertSourceRelativePathToBuiltRelativePath( i_sourceRelativePath )
...
function AssetTypeInfo.RegisterReferencedAssets( i_sourceRelativePath )
...
function AssetTypeInfo.ShouldTargetBeBuilt( i_lastWriteTime_builtAsset )
...
local function BuildAsset( i_assetInfo )
	...
	if string.find(string.lower(assetTypeInfo.type), 'shader')
	then
		assetDir = "/Shaders/"
	end

	local path_source = s_AuthoredAssetDir .. assetDir .. i_assetInfo.path
	local path_target = s_BuiltAssetDir .. string.lower(assetDir) .. assetTypeInfo.ConvertSourceRelativePathToBuiltRelativePath( i_assetInfo.path )
